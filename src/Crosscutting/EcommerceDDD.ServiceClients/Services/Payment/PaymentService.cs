...
public class PaymentService(PaymentProcessingClient paymentProcessingClient) : IPaymentService
{
	...
	public async Task RequestPaymentAsync(Guid customerId, Guid orderId, string currencyCode, double totalAmount,
		IList<PaymentProductItem> items, CancellationToken cancellationToken)
	{
		var productItems = items.Select(i => new PaymentProcessing.Models.ProductItemRequest()
		{
	...
	public async Task CancelPaymentAsync(Guid orderId, Guid paymentId, int cancellationReason, CancellationToken cancellationToken)
	{
