...
public class GetOrdersHandler : QueryHandler<GetOrders, List<OrderViewModel>>
{
    ...
    public async override Task<List<OrderViewModel>> Handle(GetOrders query, CancellationToken cancellationToken)
    {
        var orders = _querySession.Query<OrderDetails>()
            .Where(q => q.CustomerId == query.CustomerId)
            .ToList();
